...
(function ($) {
  ...
  try {
    ...
    right_sidebar.addClass('w-0').removeClass('w-100');
    sidebar_btn.on('click', function (e) {
      ...
      sub_menu_is_showed = -1;
      if(!right_sidebar.hasClass('show-sidebar')){
        right_sidebar.addClass('w-100').removeClass('w-0');
      }
      else{
        right_sidebar.addClass('w-0').removeClass('w-100');
      }
    ...
    $(".js-sidebar-btn").click(function (event) {
      event.stopPropagation();
    }); */
    var right_sidebar = $('.js-right-sidebar');
    var sidebar_btn = $('.js-sidebar-btn');
...
(function ($) {
  ...
  $(document).ready(function() {
    ...
    function updateUserStatus(name){
      if(name != ''){
        console.log('sving name ...')
        var userInfo = firebase.auth().currentUser;
        var userId = userInfo.uid;     
        firebase.database().ref('/users/' + userId+'/status').set(name).catch(function(error) {
          console.error('Error writing new message to Firebase Database', error);      //    
        });
      }
    } */
